...
export default function AdminPanel() {
  ...
  const loadUsers = async () => {
    const res = await fetch("/api/admin/get-all-users", { headers: { "x-secret": localStorage.getItem("admin_key") || "" } });
  ...
  return (
    <div className="p-6 bg-black text-green-500 font-mono min-h-screen">
      ...
      <table className="w-full border border-green-800 text-sm">
        <thead><tr className="bg-green-950">
          ...
          {users.map((u: any) => (
            <tr key={u.username} className="border-b border-green-800 hover:bg-gray-900">
              ...
              <td className="p-2 border border-green-800 flex gap-2">
                <button onClick={() => handleAction(u.username, 'reset_limit')} className="bg-blue-800 px-2 py-1">RESET</button>
                <button onClick={() => handleAction(u.username, 'toggle_lock')} className="bg-red-800 px-2 py-1">{u.locked ? 'UNLOCK' : 'LOCK'}</button>
              </td>
            </tr>
          ))}
        </tbody>
